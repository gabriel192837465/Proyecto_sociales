...
describe('admin/api-client', () => {
  ...
  describe('cuentas de alumno (sección 21)', () => {
    test('fetchUsuarios sin query', async () => {
      mockFetch(200, { usuarios: [] });
      const res = await apiClient.fetchUsuarios();
      expect(global.fetch).toHaveBeenCalledWith('/api/admin/usuarios',
        expect.objectContaining({ headers: expect.objectContaining({ 'X-Admin-Token': 'test-token' }) }));
      expect(res).toEqual({ usuarios: [] });
    });

    test('fetchUsuarios con query codifica el parámetro', async () => {
      mockFetch(200, { usuarios: [] });
      await apiClient.fetchUsuarios('lu cía');
      expect(global.fetch).toHaveBeenCalledWith(`/api/admin/usuarios?q=${encodeURIComponent('lu cía')}`,
        expect.anything());
    });

    test('setUsuarioActivo hace POST con el body correcto', async () => {
      mockFetch(200, { ok: true, usuario: { id: 'u1', activo: false } });
      const res = await apiClient.setUsuarioActivo('u1', false);
      expect(global.fetch).toHaveBeenCalledWith('/api/admin/usuarios/u1/activo', expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ activo: false })
      }));
      expect(res.usuario.activo).toBe(false);
    });
  });
});
=======
