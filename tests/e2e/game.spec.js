...
const { ADMIN_TOKEN, SUFFIX, loginDocente, loginAdmin, unirseAlumno, iniciarPartida, volverAlLobby } = require('../helpers/e2e-auth');
...
test.describe('Historia Quiz - flujo completo', () => {
  ...
  test('error si el alumno usa nombre duplicado', async ({ browser }) => {
    ...
    try {
      ...
      a1 = first.page;
      // intentionally not refactored: a2 tests duplicate name rejection (#error-nombre),
      // not a normal join flow (unirseAlumno expects #s-espera)
      a2 = await browser.newPage();
      await a2.goto('/alumno.html');
      await a2.locator('#input-nombre').fill(nombre);
      await a2.locator('#btn-entrar').click();
  ...
  test('alumno no puede cambiar su nombre a uno ya en uso', async ({ browser }) => {
    ...
    try {
      ...
      a1 = first.page;
      // intentionally not refactored: a2 joins with a valid name but then tries to rename
      // to nombreA (duplicate), which is tested differently from normal join flow
      a2 = await browser.newPage();
      await a2.goto('/alumno.html');
      await a2.locator('#input-nombre').fill(nombreB);
      await a2.locator('#btn-entrar').click();
      await expect(a2.locator('#s-espera')).toBeVisible({ timeout: 5000 });
