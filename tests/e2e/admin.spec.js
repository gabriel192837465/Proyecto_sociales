...
const { ADMIN_TOKEN, SUFFIX, loginAdmin } = require('../helpers/e2e-auth');
...
"¿Quién fue el primer presidente?";"Rivadavia";"San Martín";"Belgrano";"Moreno";A;Independencia`;
  ...
  test('contenido HTML en preguntas se renderiza como texto (sin XSS)', async ({ browser, page }) => {
    ...
    try {
      ...
      await alumno.goto('/alumno.html');
      await alumno.locator('#input-nombre').fill(`XSS_${suf}`);
      await alumno.locator('#btn-entrar').click();
