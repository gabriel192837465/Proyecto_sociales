...
import { abrirModalPregunta, abrirSelectorTipoPregunta, seleccionarTipoPregunta, guardarPregunta, borrarPregunta, confirmarBorrarPregunta, actualizarCamposTipo } from "./api.js";
import { abrirModalImport, switchImport, leerCSV, importar, cerrarModal } from "./api.js";
import { mostrarUsuarios, toggleUsuarioActivo } from "./usuarios.js";
...
const MANEJADORES = {
  ...
  "new-pregunta": (el) => abrirSelectorTipoPregunta(el.dataset.bancoId),
  ...
  "toggle-usuario-activo": (el) => toggleUsuarioActivo(el.dataset.usuarioId, el.dataset.usuarioActivo === "1"),
};
...
const ACCIONES = {
  ...
  "ver-usuarios": () => mostrarUsuarios(),
  "seleccionar-tipo": (el) => seleccionarTipoPregunta(el.dataset.tipo),
};

document.addEventListener("change", (e) => {
  if (e.target.id === "p-tipo") {
    actualizarCamposTipo();
  }
  if (e.target.id === "p-materia") {
    const otra = document.getElementById("p-materia-otra");
    if (otra) { otra.style.display = e.target.value === "__otra__" ? "block" : "none"; if (e.target.value !== "__otra__") otra.value = ""; }
  }
  if (e.target.id === "p-media-file") {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      const url = document.getElementById("p-media-url");
      if (url) url.value = String(reader.result || "");
      const mediaTipo = document.getElementById("p-media-tipo");
      if (mediaTipo) mediaTipo.value = file.type.startsWith("audio/") ? "audio" : "image";
    };
