...
function renderPreguntaRow(p, i, banco, letras) {
  return `
    <tr>
      ...
      <td><span class="epoca-badge">${escapeHTML(p.materia || 'Historia')}</span></td>
...
function renderPreguntasTable(banco) {
  ...
  const grupos = banco.preguntas.reduce((acc, p) => {
    const materia = p.materia || "Historia";
    (acc[materia] ||= []).push(p);
    return acc;
  }, {});
  return `
    <div class="card" style="padding:0;overflow:hidden">
      <table class="tabla-preguntas">
        ...
        ${Object.entries(grupos).map(([materia, preguntas]) => `
          <tbody class="materia-grupo">
            <tr><th colspan="6">${escapeHTML(materia)} <span>(${preguntas.length})</span></th></tr>
            ${preguntas.map((p, i) => renderPreguntaRow(p, i, banco, letras)).join("")}
          </tbody>`).join("")}
      </table>
    </div>
  `;
...
export function abrirSelectorTipoPregunta(bancoId) {
  if (bancoId) setBancoActual(bancoId);
  ui.showModal("modal-tipo-pregunta");
}

export function actualizarCamposTipo() {
  const tipo = document.getElementById("p-tipo")?.value || "multiple_choice";
  const media = document.getElementById("p-media-fields");
  if (media) media.style.display = tipo === "media" ? "block" : "none";
  document.querySelectorAll(".opcion-input-wrap").forEach((el, index) => {
    el.style.display = tipo === "true_false" && index > 1 ? "none" : "";
  });
  document.querySelectorAll(".correcta-radio label").forEach((el, index) => {
    el.style.display = tipo === "true_false" && index > 1 ? "none" : "";
  });
  if (tipo === "true_false") {
    ["p-opA", "p-opB"].forEach((id, i) => ui.setModalValue(id, i === 0 ? "Verdadero" : "Falso"));
    ["p-opC", "p-opD"].forEach(id => ui.setModalValue(id, ""));
    document.querySelectorAll("input[name='correcta']").forEach((radio, index) => {
      radio.disabled = index > 1;
      if (index > 1) radio.checked = false;
    });
  } else {
    document.querySelectorAll("input[name='correcta']").forEach(radio => {
      radio.disabled = false;
    });
  }
}

export function seleccionarTipoPregunta(tipo) {
  ui.setModalValue("p-tipo", tipo);
  ui.hideModal("modal-tipo-pregunta");
...
export async function abrirModalPregunta(bancoId = null, pregId = null) {
  ...
  if (pregId) {
    ...
    if (p) {
      ui.setModalValue("p-tipo", p.tipo || "multiple_choice");
      ...
      ui.setModalValue("p-media-tipo", p.mediaTipo || "image");
      ui.setModalValue("p-media-url", p.mediaUrl || "");
      const mediaFile = document.getElementById("p-media-file");
      if (mediaFile) mediaFile.value = "";
      const materia = p.materia || "Historia";
      const materiaSelect = document.getElementById("p-materia");
      const materiaOtra = document.getElementById("p-materia-otra");
      if (materiaSelect) {
        const conocida = [...materiaSelect.options].some(o => o.value === materia);
        materiaSelect.value = conocida ? materia : "__otra__";
        if (materiaOtra) { materiaOtra.value = conocida ? "" : materia; materiaOtra.style.display = conocida ? "none" : "block"; }
      }
      document.querySelectorAll("input[name='correcta']").forEach(r => r.checked = parseInt(r.value) === p.correcta);
    }
  } else {
    ui.setModalValue("p-pregunta", "");
    ["p-opA", "p-opB", "p-opC", "p-opD", "p-epoca", "p-media-url"].forEach(id => ui.setModalValue(id, ""));
    ui.setModalValue("p-tipo", "multiple_choice");
    ui.setModalValue("p-media-tipo", "image");
    ui.setModalValue("p-materia", "Historia");
    ui.setModalValue("p-materia-otra", "");
    const materiaOtra = document.getElementById("p-materia-otra");
    if (materiaOtra) materiaOtra.style.display = "none";
    const mediaFile = document.getElementById("p-media-file");
    if (mediaFile) mediaFile.value = "";
...
export async function guardarPregunta() {
  ...
  const opcionesGuardar = tipo === "true_false" ? opciones.slice(0, 2) : opciones;
  if (opcionesGuardar.some(o => !o)) { toast("Completá todas las opciones", "err"); return; }
  if (opcionesGuardar.some(o => o.length < 1)) { toast("Cada opción debe tener al menos 1 carácter", "err"); return; }
  const correcta = parseInt(document.querySelector("input[name='correcta']:checked")?.value ?? 0);
  if (correcta < 0 || correcta >= opcionesGuardar.length) { toast("Seleccioná una respuesta correcta válida", "err"); return; }
  const epoca = document.getElementById("p-epoca").value.trim() || DEFAULT_EPOCA;
  const materiaSelect = document.getElementById("p-materia");
  const materia = materiaSelect?.value === "__otra__"
    ? (document.getElementById("p-materia-otra")?.value || "").trim()
    : (materiaSelect?.value || "Historia");
  if (!materia) { toast("Elegí o escribí una materia", "err"); return; }
  const mediaTipo = tipo === "media" ? (document.getElementById("p-media-tipo")?.value || "") : "";
  const mediaUrl = tipo === "media" ? (document.getElementById("p-media-url")?.value || "").trim() : "";
  if (tipo === "media" && !mediaUrl) { toast("Ingresá la URL de la imagen o audio", "err"); return; }
  const data = { pregunta, opciones: opcionesGuardar, correcta, epoca, tipo, materia, mediaTipo, mediaUrl };
  try {
